...
func (p *Provider) doRequest(ctx context.Context, domain string, params map[string]string) ([]string, error) {
	...
	if bodyParts[0] != "OK" {
		return nil, fmt.Errorf("InviDNS request failed, expected (OK) but got (%s), url: [%s], body: %s", bodyParts[0], u, body)
	}

	return bodyParts, nil
}

// Invidns only lets you write to your subdomain.
// It must be in format subdomain.invidns.org,
// not in format subsubdomain.subdomain.invidns.org.
